...
function PWAINBackendSDK(params) {
    ...
    for (var prop in configParams) {
      if (prop in config) {
        ...
        throw (
          "Key " +
          prop +
          " is either not part of the configuration or has incorrect Key name. check the config array key names to match your key names of your config array."
        );
      }
    }
  };
  ...
  if (Object.keys(params).length > 0) {
    ...
    canWrite(__dirname + "/PayWithAmazon/config.ini", function (
      err,
      isWritable
    ) {
  ...
  var getMandatoryField = function (fieldName, parameters) {
    ...
    if (validateNotNull(value)) {
      throw (
        "Error with json message - mandatory field " +
        fieldName +
  ...
  var checkForRequiredParameters = function (parameters, fields) {
    ...
    for (var key in parameters) {
      if (!fields.hasOwnProperty(key)) {
        throw (
          "Error with json message - provided field " +
          key +
          " should not be part of input"
        );
      }
    }
  };
  ...
  var urlEncode = function (value, path) {
    ...
    if (path) {
      // no variable found as input encoded, Also we are always using path = false as input
  ...
  var getPublicKey = function () {
    if (fs.existsSync(__dirname + "/PayWithAmazon/config.ini")) {
      var iniData = ini.parse(
        fs.readFileSync(__dirname + "/PayWithAmazon/config.ini", "utf-8")
      );
  ...
  var openssl_public_encrypt = function (
    toEncrypt,
  ...
  var encryptWithPHP71 = function (K, key_length, IV, P, A, tag_length) {
    ...
    var buffer = Buffer.from(encrypted, "binary");
  ...
  var encrypt = function (K, IV, P, A, tag_length) {
    ...
    assert(
      is_string(P),
      "The data to encrypt must be null or a binary string."
    );

    assert(
      is_string(A),
      "The Additional Authentication Data must be null or a binary string."
    );

    assert(
      is_integer(tag_length),
      "Invalid tag length. Supported values are: 128, 120, 112, 104 and 96."
    );

    assert(
      [128, 120, 112, 104, 96].indexOf(tag_length) >= 0,
      "Invalid tag length. Supported values are: 128, 120, 112, 104 and 96."
    );
  ...
  this.generateSignatureAndEncrypt = function (parameters) {
    try {
      ...
      console.log("getParametersAsStringbefore");
      encryptedResponseAsString = getParametersAsString(encryptedResponse);
      console.log("enyrupted srt");
      console.log(encryptedResponseAsString);
    } catch (err) {
  ...
  this.getProcessPaymentUrl = function (parameters, redirectUrl) {
    ...
    validateNotNull(
      redirectUrl,
      "Invalid redirect URL. Please remember to input http:// or https:// as well. URL scheme"
    );
  ...
  var getFileSize = function () {
    var size = 0;
    if (
      fs.existsSync(__dirname + "/PayWithAmazon/metrics/latencyMetrics.txt")
    ) {
      var file1 = fs.statSync(
        __dirname + "/PayWithAmazon/metrics/latencyMetrics.txt"
      );
  ...
  var postMetrics = function () {
    ...
    if (
      iniData["lastFetchedTimeForPostingMetrics"] <
  ...
  this.verifySignature = function (paymentResponseMap) {
    try {
      ...
      if (
        paymentResponseMap["verificationOperationName"] ==
        "VERIFY_PROCESS_CHARGE_RESPONSE"
      ) {
        delete paymentResponseMap["verificationOperationName"];
        checkForRequiredParameters(
          paymentResponseMap,
          params_verifySignatureForProcessChargeResponse
        );
      } else if (
        paymentResponseMap["verificationOperationName"] ==
        ...
        checkForRequiredParameters(
          paymentResponseMap,
          params_verifySignatureForChargeStatus
        );
      } else {
  ...
  var createServiceUrl = function () {
    modePath = config["sandbox"]
  ...
  var invokeGet = function (parameters, endpoint, callBack) {
    try {
      ...
      httpCurlRequest.httpGet(endpoint + "?" + parameters, userAgent, function (
        err,
        response,
        body
      ) {
  ...
  var calculateSignatureAndPost = function (parameters, callBack) {
    ...
    response = invokeGet(parametersString, mwsServiceUrl, callBack);
  };

  var setParametersAndPost = function (
    parameters,
    fieldMappings,
    requestParameters,
    callBack
  ...
  this.RefundInit = function (requestParameters, callBack) {
    try {
      ...
      responseObject = setParametersAndPost(
        parameters,
        fieldMappings,
        requestParameters,
        callBack
  ...
  this.fetchTransactionDetails = function (requestParameters, callBack) {
    try {
      checkForRequiredParameters(
        requestParameters,
        params_fetchTransactionDetails
      );
      if (requestParameters["transactionIdType"] != "TRANSACTION_ID") {
        throw "Transaction Type is not supported.";
      }
      requestParameters["operationName"] =
        "SIGN_AND_ENCRYPT_GET_CHARGE_STATUS_REQUEST";
      var getChargeStatusRequest = this.generateSignatureAndEncrypt(
        requestParameters
      );
      var serviceUrl =
        "https://amazonpay.amazon.in//payments/v1/api/chargeStatus";
      responseObject = invokeGetDetails(
  ...
  this.listOrderReference = function (requestParameters, callBack) {
    try {
      if (validateNotNull(requestParameters["queryId"], "SellerOrderId")) {
        throw "Error  - mandatory field SellerOrderId cannot be found or is empty";
      }
      parameters = {};
